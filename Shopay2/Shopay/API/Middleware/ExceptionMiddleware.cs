...
namespace API.Middleware
{
    public class ExceptionMiddleware
    {
        ...
        public async Task InvokeAsync(HttpContext context)   //exception handling middleware
        {
            try
            {
                await _next(context);   //if no exception then the request moves on to another next context
            }
            catch (Exception ex)
            {
